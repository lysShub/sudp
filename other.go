...
func (r *Read) 发送速度控制包(ns int) error {
	var da []byte = []byte{uint8(ns >> 24), uint8(ns >> 16), uint8(ns >> 8), uint8(ns)}

	if da, err = packet.SecureEncrypt(da, r.secureKey); e.Errlog(err) {
		return err
	} else {
...
func (r *Read) 文件重发包(ownRec [][2]int64) error {
	if len(ownRec) == 0 {
		return nil
	}
...
func (r *Read) sendSchedulPacket(sch int64) error {
	var da []byte
	if da, err = packet.SecureEncrypt([]byte{uint8(sch >> 32), uint8(sch >> 24), uint8(sch >> 16), uint8(sch >> 8), uint8(sch)}, r.secureKey); e.Errlog(err) {
		return err
	} else {
...
func (r *Read) 接收文件信息包或结束包() (string, int64, bool, error) {
	var da []byte
	var l int
	var flag bool = true
	...
	for {
		...
		if l, err = r.conn.Read(da); err != nil {
			...
			} else if strings.Contains(err.Error(), "larger") {
				continue
			...
			if dl, bias, _, err := packet.ParsePacket(da[:l], r.key); err == nil {
				...
				if bias == 0x3FFFFF0001 { // 文件信息
					...
					if sda, err := packet.SecureDecrypt(da[:dl], r.secureKey); err == nil && len(sda) > 5 {
						flag = false

						go func() { // 回复开始包
							if rda, err := packet.SecureEncrypt(nil, r.secureKey); e.Errlog(err) {
								return
							} else {
...
func (r *Read) 文件结束包数据() ([]byte, error) {
...
func (w *Write) 发送文件信息包并接收开始包(name string, fs int64) error {
	var sda, rda []byte = []byte{uint8(fs >> 32), uint8(fs >> 24), uint8(fs >> 16), uint8(fs >> 8), uint8(fs)}, nil
	sda = append(sda, []byte(name)...)

	if sda, err = packet.SecureEncrypt(sda, w.secureKey); e.Errlog(err) {
		return err
	} else {
